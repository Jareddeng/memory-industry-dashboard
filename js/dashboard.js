// dashboard.js - Main dashboard logic with spec filtering and time range

const Dashboard = {
    state: {
        dramSpotFilter: 'all',
        dramSpotDays: 'all',
        dramContractFilter: 'all',
        nandSpotFilter: 'all',
        nandSpotDays: 'all',
        nandContractFilter: 'all'
    },
    ...
    bindEvents() {
        ...
        if (refreshBtn) refreshBtn.addEventListener('click', () => this.loadReport());

        const selector = document.getElementById('report-date-selector');
        if (selector) {
            selector.addEventListener('change', (e) => {
                if (e.target.value) this.loadReport(e.target.value);
            });
        }
    },

    bindChartFilters() {
        // DRAM Spot filter + time
        document.querySelectorAll('#dram-spot-filter .filter-tab').forEach(btn => {
            btn.addEventListener('click', () => {
                document.querySelectorAll('#dram-spot-filter .filter-tab').forEach(b => b.classList.remove('active'));
                btn.classList.add('active');
                this.state.dramSpotFilter = btn.dataset.filter;
                this.renderDramSpotChart();
            });
        });
        document.querySelectorAll('#dram-spot-time .time-tab').forEach(btn => {
            btn.addEventListener('click', () => {
                document.querySelectorAll('#dram-spot-time .time-tab').forEach(b => b.classList.remove('active'));
                btn.classList.add('active');
                this.state.dramSpotDays = btn.dataset.days;
                this.renderDramSpotChart();
            });
        });

        // DRAM Contract filter
        document.querySelectorAll('#dram-contract-filter .filter-tab').forEach(btn => {
            btn.addEventListener('click', () => {
                document.querySelectorAll('#dram-contract-filter .filter-tab').forEach(b => b.classList.remove('active'));
                btn.classList.add('active');
                this.state.dramContractFilter = btn.dataset.filter;
                this.renderDramContractChart();
            });
        });

        // NAND Spot filter + time
        document.querySelectorAll('#nand-spot-filter .filter-tab').forEach(btn => {
            btn.addEventListener('click', () => {
                document.querySelectorAll('#nand-spot-filter .filter-tab').forEach(b => b.classList.remove('active'));
                btn.classList.add('active');
                this.state.nandSpotFilter = btn.dataset.filter;
                this.renderNandSpotChart();
            });
        });
        document.querySelectorAll('#nand-spot-time .time-tab').forEach(btn => {
            btn.addEventListener('click', () => {
                document.querySelectorAll('#nand-spot-time .time-tab').forEach(b => b.classList.remove('active'));
                btn.classList.add('active');
                this.state.nandSpotDays = btn.dataset.days;
                this.renderNandSpotChart();
            });
        });

        // NAND Contract filter
        document.querySelectorAll('#nand-contract-filter .filter-tab').forEach(btn => {
            btn.addEventListener('click', () => {
                document.querySelectorAll('#nand-contract-filter .filter-tab').forEach(b => b.classList.remove('active'));
                btn.classList.add('active');
                this.state.nandContractFilter = btn.dataset.filter;
                this.renderNandContractChart();
            });
        });
    },
    ...
    renderOverviewCharts() {
        ChartRenderer.renderOverviewChart('overview-dram-spot', DataLoader.getDramSpotData(), { filter: 'all' });
        ChartRenderer.renderOverviewChart('overview-nand-spot', DataLoader.getNandSpotData(), { filter: 'all' });
    },
    ...
    renderDramSpotChart() {
        const data = DataLoader.getDramSpotData();
        this.updateChartMeta('dram-spot-meta', data);
        ChartRenderer.renderSpotChart('dram-spot-chart', data, {
            filter: this.state.dramSpotFilter,
            days: this.state.dramSpotDays
        });
    },

    renderDramContractChart() {
        const data = DataLoader.getDramContractData();
        this.updateChartMeta('dram-contract-meta', data);
        ChartRenderer.renderContractChart('dram-contract-chart', data, {
            filter: this.state.dramContractFilter
        });
    },

    renderNandSpotChart() {
        const data = DataLoader.getNandSpotData();
        this.updateChartMeta('nand-spot-meta', data);
        ChartRenderer.renderSpotChart('nand-spot-chart', data, {
            filter: this.state.nandSpotFilter,
            days: this.state.nandSpotDays
        });
    },

    renderNandContractChart() {
        const data = DataLoader.getNandContractData();
        this.updateChartMeta('nand-contract-meta', data);
        ChartRenderer.renderContractChart('nand-contract-chart', data, {
            filter: this.state.nandContractFilter
        });
    },

    updateChartMeta(metaId, data) {
        const el = document.getElementById(metaId);
        if (!el || !data) return;
        const freq = data.data_frequency || '';
        const range = data.time_range || '';
        const source = data.data_source || '';
        el.innerHTML = `<span class="meta-chip">${freq}</span> <span class="meta-chip">${range}</span> <span class="meta-chip source">${source}</span>`;
    },
                ...
                return `
                    <div class="stock-detail-card">
                        ...
                        <div class="detail-stats">
                            <div class="detail-stat"><span class="detail-stat-label">昨收</span><span class="detail-stat-value">${stock.previous_close.toLocaleString()}</span></div>
                            <div class="detail-stat"><span class="detail-stat-label">52周高</span><span class="detail-stat-value">${stock.high_52w.toLocaleString()}</span></div>
                            <div class="detail-stat"><span class="detail-stat-label">52周低</span><span class="detail-stat-value">${stock.low_52w.toLocaleString()}</span></div>
                            <div class="detail-stat"><span class="detail-stat-label">成交量</span><span class="detail-stat-value">${(stock.volume / 1000000).toFixed(1)}M</span></div>
                        </div>
                    </div>
                `;
            }).join('');
    ...
    renderHbmTimeline() {
        ...
        const statusMap = { 'completed': '已完成', 'in_progress': '进行中', 'upcoming': '待开始' };
    ...
    renderExpansion(company) {
        ...
        const companyMap = { 'sk-hynix': 'SK Hynix', 'samsung': 'Samsung Electronics', 'micron': 'Micron Technology' };
        const companyData = expansionData.companies.find(c => c.name === companyMap[company]);
        if (!companyData) {
            container.innerHTML = '<p class="placeholder">暂无数据</p>';
            return;
        }
        const statusClassMap = { '建设中': 'status-construction', '规划': 'status-planning', '扩产中': 'status-expanding' };
        ...
        if (!hasContent) {
            ...
            container.innerHTML = `
                ...
                ${report.risk_alerts && report.risk_alerts.length > 0 ? `
                    <div style="margin-top:12px;"><strong>⚠️ 风险提示:</strong>
                    <ul>${report.risk_alerts.map(r => `<li>${r}</li>`).join('')}</ul></div>
                ` : ''}
            `;
        }
    },
    ...
    renderReportHtml(report) {
        return `
            <div class="report-content-rendered">
                ...
                <h3>市场综述</h3><p>${report.summary || '暂无内容'}</p>
                <h3>今日交易评价</h3><p>${report.market_assessment || '暂无内容'}</p>
                <h3>风险提示</h3>
                ${report.risk_alerts && report.risk_alerts.length > 0
                    ? `<ul>${report.risk_alerts.map(r => `<li>${r}</li>`).join('')}</ul>`
                    : '<p>暂无</p>'}
                <h3>展望</h3><p>${report.outlook || '暂无内容'}</p>
            </div>
        `;
    },
